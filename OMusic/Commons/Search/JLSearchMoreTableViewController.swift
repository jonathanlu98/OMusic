...
class JLSearchMoreTableViewController: UITableViewController,UIGestureRecognizerDelegate {
    ...
    private func handleTableView() {
        ...
        case .album:
            ...
            self.bindData(cellId: Search_AlbumCellID, cellType: OMSearchListTableViewAlbumCell.self)
            break
        case .artist:
            self.tableView.register(UINib.init(nibName: Search_ArtistCellID, bundle: nil), forCellReuseIdentifier: Search_ArtistCellID)
            self.bindData(cellId: Search_ArtistCellID, cellType: OMSearchListTableViewArtistCell.self)
            break
        case .track:
        ...
        self.data.bind(to: self.tableView.rx.items(cellIdentifier: cellId, cellType: cellType)) {index,item,cell in
            switch item {
                ...
                case .ArtistSectionItem(let aritem):
                    (cell as! OMSearchListTableViewArtistCell).item = aritem
                    (cell as! OMSearchListTableViewArtistCell).setupUI()
                    
                case .AlbumSectionItem(let alitem):
                    (cell as! OMSearchListTableViewAlbumCell).item = alitem
                    (cell as! OMSearchListTableViewAlbumCell).setupUI()
