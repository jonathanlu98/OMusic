...
extension UIImageView {
    ...
    /// 图片加载
    /// - Parameter url: 图片url
    func fetchImage(_ url:URL?, placeholderImage: UIImage? = nil, _ completionBlock: fetchImageCompletionBlock?) {
        if placeholderImage == nil {
            self.alpha = 0
        }
        if (url != nil) {
            self.sd_setImage(with: url, placeholderImage: placeholderImage, options: .retryFailed) { [weak self] image, error, type, uurl in
...
extension UIView {
    static func loadFromXib() -> Self? {
        let views = Bundle.main.loadNibNamed(NSStringFromClass(self).components(separatedBy: ".").last!, owner: self, options: nil)
        if !OMArrayIsEmpty(views) {
